...
#if _WIN32 || _WIN64
#if _WIN64
#define WIN_ENVIRONMENT64
#include <nvml.h>
#else
#define WIN_ENVIRONMENT32
#endif
#endif
...
#define LINUX_ENVIRONMENT64
#else
#define LINUX_ENVIRONMENT32
#endif
#endif
...
void init_device (int device_number, int show_prop)
{
  ...
  if (device_number >= device_count)
  {
	...
	printf ("(If you didn't select a device this is probably a driver problem)\n\n");
	exit (2);
  }
...
#if CUDART_VERSION >= 7000	//CUDA >=7 Does not support compute_1.x
	if (g_dev.major == 1 && g_dev.minor <= 3)
	{
...
check (int q)
{
  ...
  printf ("You may experience a small delay on 1st startup to due to Just-in-Time Compilation\n\n");
  do
  {				/* while (restarting) */
